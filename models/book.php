...
class Book {
	...
	public function __toString() {
		return  'book:'.$this->bookId.','.$this->name.','.$this->place.''.$this->price.','.$this->ISBN.','.$this->sunccoNo.','.$this->addTime.','.$this->status.','.$this->expiredDays;
	}
...
class BookManager {
	...
	public static function addBook( Book &$book ) {
		...
		try {
			...
			$sql  = "insert into book (name,isbn,suncco_no,price,add_time,place) values (?,?,?,?,?,?)";
	...
	public static function getBook( $bookId ) {
		...
		try {
			...
			if ( $row = $stmt->fetch() ) {
				...
				$tempBook->price     = $row['price'];
				$tempBook->place     = $row['place'];
